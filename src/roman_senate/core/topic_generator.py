...
simulation based on the selected year, using the configured LLM provider.
"""
...
# Topic categories
TOPIC_CATEGORIES = [
    "Military funding",       # Military budget decisions
    "Public projects",        # Infrastructure and public works
    "Personal ego projects",  # Vanity projects for senators
    "Military campaigns",     # War decisions
    "Class rights",           # Social class issues
    "General laws",           # Legal reforms
    "Trade relations",        # Economic policy
    "Foreign relations",      # Diplomacy
    "Religious matters",      # Religious policy and rites
    "Economic policy"         # Finances and taxation
]
...
async def generate_topics_for_year(year: int, count: int = 10) -> Dict[str, List[str]]:
    """
    ...
    # Create a prompt for LLM to generate historically accurate topics
    prompt = f"""
    ...
    try:
        ...
        console.print(f"[dim]Generating topics using {LLM_PROVIDER} ({LLM_MODEL})...[/]")
        ...
        try:
            ...
            console.print(f"[bold yellow]Warning: Could not parse LLM response: {e}[/]")
...
async def get_topics_for_year(year: int, count: int = 10) -> Dict[str, List[str]]:
    """
    ...
    """
    try:
        return await generate_topics_for_year(year, count)
    except Exception as e:
