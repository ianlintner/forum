...
# Latin integration patterns by archetype
ARCHETYPE_LATIN_PATTERNS = {
    "traditionalist": {
        "frequency": 0.8,          # How often Latin is used
        "formality": 0.9,          # How formal the Latin phrases are
        "antiquity": 0.8,          # Preference for older/archaic forms
        "legal_terms": 0.8,        # Use of legal terminology
        "religious_terms": 0.7,    # Use of religious terminology
        "philosophical_terms": 0.5  # Use of philosophical terminology
    },
    "pragmatist": {
        "frequency": 0.5,
        "formality": 0.6,
        ...
        "legal_terms": 0.5,
        "religious_terms": 0.3,
        "philosophical_terms": 0.2,
        "commercial_terms": 0.8,   # Business and trade terms
        "administrative_terms": 0.7 # Government administration terms
    },
    "philosopher": {
        "frequency": 0.7,
        "formality": 0.8,
        ...
        "legal_terms": 0.4,
        "religious_terms": 0.5,
        "philosophical_terms": 0.9,
        "greek_loanwords": 0.8     # Greek-derived philosophical terms
    },
    "populist": {
        "frequency": 0.4,
        "formality": 0.3,
        ...
        "legal_terms": 0.2,
        "religious_terms": 0.5,
        "philosophical_terms": 0.1,
        "common_phrases": 0.9,     # Widely understood expressions
        "emotive_terms": 0.8       # Emotionally charged terminology
    },
    "militarist": {
        "frequency": 0.6,
        "formality": 0.5,
        ...
        "religious_terms": 0.4,
        "philosophical_terms": 0.1,
        "military_terms": 0.9,     # Military and command terminology
        "direct_imperatives": 0.7  # Command forms
    }
}

# Speech structure tendencies by archetype
ARCHETYPE_STRUCTURE_WEIGHTS = {
    "traditionalist": {
        "exordium_length": 0.7,      # Length of introduction
        "narratio_detail": 0.8,      # Detail in presenting facts
        "partitio_presence": 0.9,    # Likelihood of including outline
        "confirmatio_strength": 0.7, # Emphasis on positive arguments
        "refutatio_strength": 0.6,   # Emphasis on refuting counterarguments
        "peroratio_formality": 0.9   # Formality of conclusion
    },
    "pragmatist": {
        "exordium_length": 0.4,
        "narratio_detail": 0.7,
        "partitio_presence": 0.8,
        "confirmatio_strength": 0.8,
        "refutatio_strength": 0.7,
        "peroratio_formality": 0.5
    },
    "philosopher": {
        "exordium_length": 0.6,
        "narratio_detail": 0.6,
        "partitio_presence": 0.9,
        "confirmatio_strength": 0.9,
        "refutatio_strength": 0.8,
        "peroratio_formality": 0.7
    },
    "populist": {
        "exordium_length": 0.5,
        "narratio_detail": 0.4,
        "partitio_presence": 0.3,
        "confirmatio_strength": 0.6,
        "refutatio_strength": 0.5,
        "peroratio_formality": 0.3
    },
    "militarist": {
        "exordium_length": 0.3,
        "narratio_detail": 0.5,
        "partitio_presence": 0.6,
        "confirmatio_strength": 0.8,
        "refutatio_strength": 0.7,
        "peroratio_formality": 0.5
    }
...
def generate_archetype_parameters(senator: Dict[str, Any], archetype_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Add Latin patterns
    merged_params["latin_patterns"] = merge_latin_patterns(
        ARCHETYPE_LATIN_PATTERNS[primary],
        ARCHETYPE_LATIN_PATTERNS[secondary],
        primary_weight,
        secondary_weight
    )
    
    # Add speech structure weights
    merged_params["structure_weights"] = merge_structure_weights(
        ARCHETYPE_STRUCTURE_WEIGHTS[primary],
        ARCHETYPE_STRUCTURE_WEIGHTS[secondary],
        primary_weight,
        secondary_weight
    )
...
def merge_latin_patterns(primary_patterns: Dict[str, float], secondary_patterns: Dict[str, float],
                        primary_weight: float, secondary_weight: float) -> Dict[str, float]:
    """Merge Latin usage patterns from two archetypes."""
    merged = {}
    
    # Add all primary patterns
    for pattern, value in primary_patterns.items():
        merged[pattern] = value * primary_weight
    
    # Add or update with secondary patterns
    for pattern, value in secondary_patterns.items():
        if pattern in merged:
            merged[pattern] += value * secondary_weight
        else:
            merged[pattern] = value * secondary_weight
    
    # Normalize to ensure some values are still close to 1.0
    max_value = max(merged.values())
    if max_value > 0:
        scaling_factor = 1.0 / max_value
        for pattern in merged:
            merged[pattern] *= scaling_factor
    
    return merged

def merge_structure_weights(primary_weights: Dict[str, float], secondary_weights: Dict[str, float],
                           primary_weight: float, secondary_weight: float) -> Dict[str, float]:
    """Merge speech structure weights from two archetypes."""
    merged = {}
    
    # Add all primary weights
    for element, weight in primary_weights.items():
        merged[element] = weight * primary_weight
    
    # Add or update with secondary weights
    for element, weight in secondary_weights.items():
        if element in merged:
            merged[element] += weight * secondary_weight
        else:
            merged[element] = weight * secondary_weight
    
    # No need to normalize structure weights as they're used as-is
