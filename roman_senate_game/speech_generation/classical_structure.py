...
from .historical_context import get_historically_appropriate_address
...
def generate_speech_structure(senator: Dict, topic: str, archetype_params: Dict,
    ...
    structure_weights = archetype_params.get("structure_weights", {})
    ...
    historical_figures = []
    if historical_context.get("figures"):
        historical_figures = [f["name"] for f in historical_context["figures"]]
    
    historical_events = []
    if historical_context.get("events"):
        historical_events = [e["name"] for e in historical_context["events"]]
    
    values = []
    if historical_context.get("values"):
        values = [v["name"] for v in historical_context["values"]]
    
    # Define template variables for formatting
    template_vars = {
        "address": address,
        "topic": topic,
        "historical_figure": random.choice(historical_figures) if historical_figures else "our ancestors",
        "historical_event": random.choice(historical_events) if historical_events else "our history",
        "value": random.choice(values) if values else "Roman virtue",
        # Additional variables would be filled based on the specific speech content
    }
    ...
    for part_name, part_info in SPEECH_PARTS.items():
        ...
        weight = structure_weights.get(f"{part_name}_presence", 0.5)
        include_part = part_info["required"] or random.random() < weight
        ...
        if include_part:
            ...
            length_factor = structure_weights.get(f"{part_name}_length", 0.5)
            adjusted_max = min_len + int(round((max_len - min_len) * length_factor))
            length = random.randint(min_len, max(min_len, adjusted_max))
...
def expand_speech_structure(speech_structure: Dict, topic: str, 
    ...
    # use more sophisticated natural language generation techniques
    ...
    for part_name, part_data in speech_structure.items():
        ...
        # In a full implementation, this would involve more sophisticated text generation
        try:
            content = template.format(**variables)
        except KeyError:
            # Fallback if template has variables we haven't defined
            content = f"[{part_name} about {topic}]"
